...
module.exports.generate = async (set) => {
  ...
  fieldname.data.forEach((item) => {
    ws.cell(fieldname.row, fieldname.col)
      .string(item)
      .style(option.table.thStyle);
    fieldname.col++;
  });
  ...
  for (let i = 0; i < tabledata.data.length; i++) {
    ...
    for (let j = 0; j <= maxLoop; j++) {
      ...
      switch (typeof data) {
        case "number":
          ws.cell(tabledata.row, tabledata.col)
            .number(data)
            .style(option.table.tdStyle);
          break;
        default:
          ws.cell(tabledata.row, tabledata.col)
            .string(data)
  ...
  // fs.copyFileSync(
  //   path.join(__dirname, "..", "xfiles", "base.xlsx"),
  //   path.join(__dirname, "..", "xfiles", filename)
  // );
